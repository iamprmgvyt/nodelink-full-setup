...
function makeSubkeys(keyBytes) {
    const high = ((keyBytes[0] ?? 0) << 24) |
        ((keyBytes[1] ?? 0) << 16) |
        ((keyBytes[2] ?? 0) << 8) |
        (keyBytes[3] ?? 0);
    const low = ((keyBytes[4] ?? 0) << 24) |
        ((keyBytes[5] ?? 0) << 16) |
        ((keyBytes[6] ?? 0) << 8) |
        (keyBytes[7] ?? 0);
    let c = 0;
...
function desBlockAt(input, offset, subkeys, decrypt, out) {
    const high = ((input[offset] ?? 0) << 24) |
        ((input[offset + 1] ?? 0) << 16) |
        ((input[offset + 2] ?? 0) << 8) |
        (input[offset + 3] ?? 0);
    const low = ((input[offset + 4] ?? 0) << 24) |
        ((input[offset + 5] ?? 0) << 16) |
        ((input[offset + 6] ?? 0) << 8) |
        (input[offset + 7] ?? 0);
